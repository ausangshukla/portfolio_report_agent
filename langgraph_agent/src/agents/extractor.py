...
class ExtractorNode:
    ...
    def __init__(self, llm):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert financial analyst. Your task is to extract
             ...
             'content': The extracted and summarized content for the section.
             'references': A list of dictionaries, each with 'document' (filename) and 'location' (page/row).
             """),
    ...
    def extract(self, state: AgentState) -> Dict[str, Any]:
        ...
        print(f"--- ExtractorNode: Extracting for section '{current_section}' ---")
        print(f"ExtractorNode: Input documents for '{current_section}':\n{formatted_documents[:500]}...") # Log first 500 chars of documents
        ...
        try:
            ...
            error_message = f"ExtractorNode: Error during extraction for '{current_section}': {e}"
            print(error_message)
            return {
                "messages": state.get("messages", []) + [
...
if __name__ == "__main__":
    ...
    initial_state: AgentState = {
        ...
        "sections_to_process": ["Overview", "Financial Review"],
        "completed_sections": [],
